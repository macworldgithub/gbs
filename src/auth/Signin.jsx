...
const Signin = () => {
  ...
  const handleSignin = async () => {
    ...
    try {
      ...
      if (token && user) {
        ...
        // ✅ Persist credentials for auto-login refresh
        try {
          const credentials = { identifier: trimmedIdentifier, password };
          await AsyncStorage.setItem("userCredentials", JSON.stringify(credentials));
          console.log("💾 Credentials saved for auto-login");
        } catch (storageErr) {
          console.error("❌ Failed to save credentials:", storageErr);
        }
        // if (useBiometrics && biometricCapable && token) {
        //   await setSession(userData, { requireBiometrics: true });
        //   await setBiometricsEnabled(true);
  ...
  return (
    <KeyboardAvoidingView
      ...
      behavior={Platform.OS === "ios" ? "padding" : undefined}
    >
      <ScrollView contentContainerStyle={tw`flex-1  justify-center px-6 pt-10`}>
        <Text style={tw`text-2xl font-bold text-gray-900 mb-1`}>
