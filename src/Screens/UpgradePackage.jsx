...
import { saveCurrentRoleBeforeUpgrade } from "../utils/packageTracker";
...
const UpgradePackage = ({ navigation }) => {
  ...
  const refreshUserAndRoles = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);

      // Force refresh from storage + backend
      const freshUserData = await getUserData(); // This already tries AsyncStorage + secure session
      if (!freshUserData?.token) {
        setError("Session expired. Please login again.");
        return;
      }

      const currentId = freshUserData?.activatedPackage?.role?._id;
      setCurrentRoleId(currentId);

      // Re-fetch roles (in case they changed)
      const res = await axios.get(`${API_BASE_URL}/roles`, {
        headers: { Authorization: `Bearer ${freshUserData.token}` },
      });

      setRoles(res.data || []);
      console.log("✅ User & roles refreshed. Current role:", currentId);
    } catch (e) {
      console.log(
        "[UpgradePackage] refresh error:",
        e.response?.data || e.message,
      );
      setError("Failed to refresh data. Pull down or go back and try again.");
    } finally {
      setLoading(false);
    }
  }, []);

  // Initial load + AppState listener for returning from browser
  useEffect(() => {
    refreshUserAndRoles();
    ...
    const subscription = AppState.addEventListener("change", (nextAppState) => {
      if (appState.match(/inactive|background/) && nextAppState === "active") {
        console.log(
          "🔄 App became active again (returned from browser) — refreshing...",
        );
        refreshUserAndRoles();
      }
  ...
  const upgradeToRole = async (role) => {
    try {
      ...
      const isRole = currentRoleId ? true : false;
      ...
      // ✅ Pass isRole in URL also
      const url = `https://gbs-stripe.vercel.app/?token=${encodeURIComponent(
        token,
      )}&roleId=${encodeURIComponent(roleId)}&isRole=${encodeURIComponent(
        isRole,
      )}`;

      console.log("Redirect URL:", url);

      // ✅ Save current role state before redirecting to Stripe
      await saveCurrentRoleBeforeUpgrade();
      ...
      <ScrollView style={tw`flex-1 px-4 py-4`}>
        ...
        {!loading &&
          roles
            .filter(
              (role) =>
                role.label !== "Chairman's Club" && role.label !== "Basic Tier",
            )
            .map((role) => (
              <View
                key={role._id}
              ...
              >
                ...
                <View style={tw`flex-row justify-between`}>
                  ...
                  <TouchableOpacity
                    ...
                    onPress={() => {
                      console.log("Button pressed for role:", role._id);
                      upgradeToRole(role);
                    }}
                  >
