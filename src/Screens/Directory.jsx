...
export default function MembersDirectory({ navigation }) {
  ...
  const handleSearch = (text) => {
    ...
    setSearchTimeout(setTimeout(() => searchUsers(text, selectedState), 500));
  };
  ...
  return (
    <View style={tw`flex-1 bg-gray-50 px-4 pt-4`}>
      <Text style={tw`text-lg font-bold text-black text-center mb-4 mt-12`}>
        Members Directory
      </Text>

      {/* Search Bar */}
      <View
        style={tw`flex-row items-center bg-white shadow rounded-full px-4 py-2 mb-4`}
      >
        ...
        {["All", "VIC", "NSW", "QLD", "SA"].map((filter) => (
          <TouchableOpacity
            ...
            style={tw`px-4 py-2 rounded-full ${
              selectedState === filter ? "bg-red-500" : "bg-white"
            } shadow`}
            onPress={() => handleStateFilter(filter)}
          >
            <Text
              style={tw`text-sm ${
                selectedState === filter ? "text-white" : "text-gray-600"
              }`}
            >
              {filter}
            </Text>
      ...
      {loading ? (
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View
              style={tw`flex-row items-center bg-white rounded-xl p-4 mb-3 shadow`}
            >
              <Image
                source={item.image}
   ...
   <Text style={tw`text-black`}>State: 
  <Text style={tw`text-red-500`}> {item.state ? item.state : "N/A"}</Text>
</Text>
              ...
              <TouchableOpacity
                style={tw`ml-3 bg-red-500 px-3 py-2 rounded-full`}
                onPress={() => openChat(item)}
              >
