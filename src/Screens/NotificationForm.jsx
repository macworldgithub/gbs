...
export default function NotificationForm({ notification, onSubmit, onCancel, isLoading = false }) {
  ...
  useEffect(() => {
    if (notification) {
      setFormData({
        ...
        endDate: notification.endDate ? new Date(notification.endDate) : new Date(),
      });
    }
  }, [notification]);
  ...
  return (
    <ScrollView style={tw`flex-1 bg-white p-4`}>
      ...
      <DateTimePicker
        ...
        mode="datetime"
        onChange={(e, date) => date && setFormData((p) => ({ ...p, startDate: date }))}
      />
      ...
      <DateTimePicker
        ...
        mode="datetime"
        onChange={(e, date) => date && setFormData((p) => ({ ...p, endDate: date }))}
      />
      ...
      <View style={tw`flex-row items-center mt-4`}>
        ...
        <Switch
          ...
          onValueChange={(val) => setFormData((p) => ({ ...p, SendToAll: val }))}
        />
      </View>
