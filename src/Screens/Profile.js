...
import { View, Text, Image, TouchableOpacity, FlatList, Alert, PermissionsAndroid, Platform } from 'react-native';
...
const menuItems = [
  { id: '1', label: 'Edit Profile', icon: 'user-edit', navigateTo: 'EditProfile' },
  { id: '2', label: 'Account Security', icon: 'shield-alt', navigateTo: 'AccountSecurity' },
  { id: '3', label: 'Scan Member', icon: 'qrcode', navigateTo: 'QRCodeScreen' },
  { id: '4', label: 'Payment Method', icon: 'credit-card', navigateTo: 'PaymentSettings' },
  { id: '5', label: 'General Settings', icon: 'cogs', navigateTo: 'GeneralSetting' },
...
const Profile = () => {
  ...
  const userId = '4535'; // Replace with actual userId if needed
 ...
 const uploadImage = async (file) => {
  ...
  try {
    ...
    const res = await axios.get('http://localhost:9000/user/profile-picture/upload-url', {
      params: {
        fileName,
        fileType,
        userId,
  ...
  return (
    <View style={tw`flex-1 bg-gray-100 mt-10`}>
      ...
      <View style={tw`items-center mt-6 mb-4`}>
        ...
        <Text style={tw`text-lg font-bold text-gray-900 mt-1`}>Franklin Clinton</Text>
