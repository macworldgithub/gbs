...
export default function Home() {
  ...
  // Guest banner countdown state
  // guestExpiry: number (ms) when guest session expires (from AsyncStorage userData.guestExpiry)
  // guestRemainingStr: computed human friendly remaining time (updates every second)
      ...
      try {
        ...
        if (parsedUserData?.isGuest) {
          ...
          if (expiry && Date.now() > expiry) {
            ...
            await AsyncStorage.removeItem("guestExpiry"); // Clean up
            setGuestExpiry(null);
          }
        }
      } catch (e) {
  ...
  // Countdown effect for guestRemainingStr (updates every second)
  useEffect(() => {
    if (!guestExpiry) {
    ...
    <View style={tw`px-4 pt-6 mt-10`}>
      ...
      <View style={tw`flex-row justify-between items-center mb-4`}>
        ...
        <View style={tw`flex-row items-center`}>
          ...
          {/* Optional other header controls (kept as in your original file) */}
        </View>
      </View>

      {/* Guest countdown banner (visible when guestExpiry is set) */}
      {guestExpiry && guestRemainingStr ? (
        ...
        >
          ...
          <Text style={tw`text-sm text-red-600 font-semibold`}>Upgrade</Text>
        </TouchableOpacity>
      ) : null}
  ...
  // Main render: list + drawer + modals
  return (
    <View style={{ flex: 1 }}>
      <FlatList
        keyboardShouldPersistTaps="always"
      ...
      {/* Create Event Modal (kept commented as in your original file) */}
      {/*
      <Modal visible={createEventModalVisible} transparent animationType="fade">
        <View
          style={tw`flex-1 bg-black bg-opacity-50 justify-center items-center`}
      ...
      */}
      ...
      {/* Date pickers that may be used with the create event modal */}
      {showStartDatePicker && (
        <DateTimePicker
          value={eventForm.startDate}
          mode="date"
