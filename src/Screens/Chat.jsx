...
export default function Chat({ navigation }) {
  ...
  const sendMediaMessage = async (file) => {
    ...
    // Fix: Use mimeType first, then fall back to type or default
    ...
    if (!allowedMimeTypes.includes(mimeType)) {
      ...
      Alert.alert("Error", `Unsupported file type: ${mimeType}`);
      return;
    }
    ...
    const fileName = file.fileName || `media-${Date.now()}.${extension}`;
  ...
  const handleCamera = async () => {
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        const sizeInMB = (capturedImage.fileSize || 0) / (1024 * 1024);
        if (sizeInMB > 10) {
          Alert.alert("Error", "Captured image exceeds 10MB limit");
          return;
        }
  ...
  return (
    ...
    >
      <View style={tw`flex-1 bg-white pt-8 pb-2`}>
        {/* Header */}
        <View
          style={tw`flex-row items-center justify-start px-4 py-3 border-b`}
        >
          <TouchableOpacity onPress={() => navigation.goBack()}>
            <Ionicons name="arrow-back" size={24} />
          </TouchableOpacity>
